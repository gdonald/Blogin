...
//
// Compilation happens up front, not on first use, so rendering is a pure
...
class TemplateStore {
public:
  // Earlier paths win, so a site's own layout overrides a theme's.
...
//
// Shared by every rendering thread, so it locks. Lookups are few per page and
// nearly all reads, so a shared lock costs little. One cache per thread would
// render the site's chrome once per thread instead of once.
...
public:
  ...
  // count that says whether the site's chrome was built once or once per page.
