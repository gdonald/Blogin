---
title: subject / subject-bang
slug: subject
order: 8
description: subject defines the primary value under test in a describe or context block. It is implemented as a let under the name :subject, so all of let's...
toc: true
---
`subject` defines the primary value under test in a `describe` or `context` block. It is implemented as a [`let`](/docs/BDD-Behave/let) under the name `:subject`, so all of `let`'s lazy-and-memoized semantics apply.
