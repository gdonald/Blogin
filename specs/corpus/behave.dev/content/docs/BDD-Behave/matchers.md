...
slug: matchers
order: 15
description: expect(...).to.be(...) is built on top of a small Matcher role. The expected value passed to .be(...) is either:
toc: true
---
...
[Custom Matchers](/docs/BDD-Behave/custom-matchers). `define-matcher` produces a matcher
...
`Diff:` section described in [Diff Output](/docs/BDD-Behave/diff).
...
[Matchers › IncludeMatcher](/docs/BDD-Behave/matchers#includematcher-built-in)).
...
without writing a class), see [Custom Matchers](/docs/BDD-Behave/custom-matchers).
...
[Composable Matchers](/docs/BDD-Behave/composable-matchers) for details.
