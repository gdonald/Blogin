...
slug: configuration
order: 29
description: Behave loads two Raku-based configuration files at startup and merges them with your CLI flags. This is the standard way to keep team-wide and...
toc: true
---
...
| `$config.parallel-mode`        | `--parallel-mode`  | `'isolated'` (default), `'lpt'`, or `'queue'`. See [Parallel Execution](/docs/BDD-Behave/parallel#parallel-mode-parallel-mode). |
| `$config.parallel-retry`       | `--parallel-retry` | Non-negative Int. Per-shard retry budget when a worker crashes (exit > 1). See [Per-shard retry](/docs/BDD-Behave/parallel#per-shard-retry-on-worker-crash-parallel-retry). |
