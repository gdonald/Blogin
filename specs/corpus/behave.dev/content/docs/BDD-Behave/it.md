...
slug: it
order: 5
description: it defines a single example. The first argument is a human-readable description, the second is a block containing the test body.
toc: true
---
...
renders as `⮑  'should be (1, 2, 3)'`. If no matcher runs in the block, Behave falls back to a placeholder of the form `example at <basename>:<line>`. See [`subject` / `is-expected`](/docs/BDD-Behave/subject#one-liner-it-form) for details.
...
Mark an example as pending with the [`pending`](/docs/BDD-Behave/pending) helper. While iterating, you can also comment out the body or leave the assertion unwritten until you implement the behavior.
...
See [`let`](/docs/BDD-Behave/let) for the various ways to define and consume let values.
