...
slug: watch
order: 26
description: Watch mode keeps behave running between runs. When a file under lib/ or specs/ changes, Behave figures out which spec files are affected and re-runs only...
toc: true
---
...
| `a`     | Re-run every spec.                                                                                      |
| `f`     | Re-run only previously-failed examples (uses `.behave-failures`, see [Retry and Only-Failures](/docs/BDD-Behave/retry)). |
