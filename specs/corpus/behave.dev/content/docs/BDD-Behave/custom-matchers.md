...
order: 16
description: When the built-in matchers don't quite say what you mean, define your own. A custom matcher is just a small bundle of callbacks: a match predicate plus...
toc: true
---
...
[Matcher](/docs/BDD-Behave/matchers) machinery used by every built-in matcher.
...
files can reach it through [`matcher(...)`](#matcher-name-args-lookup).
...
- If you omit `failure-message`, the failure renders through the structured
  ...
  [diff](/docs/BDD-Behave/diff) section, exactly like `BeMatcher`.

This means custom matchers compose with everything else the runner already
does: `.not`, the
[matcher architecture](/docs/BDD-Behave/matchers), [aggregate-failures](/docs/BDD-Behave/aggregate-failures),
and one-liner [auto-descriptions](/docs/BDD-Behave/subject#one-liner-it-form).
