...
slug: migrations
order: 4
description: ORM::ActiveRecord includes commands to migrate your database. Migrations include adding and removing tables as well as adding and removing columns and...
toc: true
---
...
DDL produced is adapter-aware (see [Adapters](/docs/ORM-ActiveRecord/adapters) for the per-engine
differences).
...
| `:jsonb`      | Binary JSON. `JSONB` on PostgreSQL; maps to `JSON` on MySQL; stored as text on SQLite. Required for the [`@>` / key-existence operators](/docs/ORM-ActiveRecord/queries#json-jsonb-predicate-operators). |
...
hits this. See [Errors &raquo; X::IrreversibleMigration](/docs/ORM-ActiveRecord/errors#x-irreversiblemigration).
...
[Database tasks](/docs/ORM-ActiveRecord/db-tasks); with no arguments the tool runs `db:migrate`.
...
This is the machinery behind parallel test runs — see [Tests](/docs/ORM-ActiveRecord/tests).
