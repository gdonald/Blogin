...
slug: callbacks
order: 24
description: ORM::ActiveRecord supports callbacks that can be performed during various life cycle events.
toc: true
---
...
`after-save-commit`) — see [Transactional Callbacks](/docs/ORM-ActiveRecord/transactions#transactional-callbacks).
...
ORM::ActiveRecord does not ship Observers. Callbacks cover the same ground:
