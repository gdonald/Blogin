...
slug: tests
order: 43
description: The suite has two halves: the prove6 tests under t/ and the behave specs under specs/. Both run against PostgreSQL, MySQL, and SQLite.
toc: true
---
...
`config/application.json` when it is unset (see [Adapters](/docs/ORM-ActiveRecord/adapters) for the
...
use `active-record … --parallel` (see [Migrations » Parallel test databases](/docs/ORM-ActiveRecord/migrations#parallel-test-databases)):
