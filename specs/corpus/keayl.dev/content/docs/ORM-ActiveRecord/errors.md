...
slug: errors
order: 39
description: ORM::ActiveRecord exposes two complementary error surfaces:
toc: true
---
...
| `X::StrictValidationFailed`| validator declared with `:strict` — see [Validator Options &raquo; strict](/docs/ORM-ActiveRecord/options#strict) |
...
wrong. See [Persistence](/docs/ORM-ActiveRecord/persistence) for the full quiet-vs-loud
breakdown.
...
you want a writable record. See [Aggregation &raquo; readonly](/docs/ORM-ActiveRecord/aggregation#readonly).
...
manual exception plumbing. See [Migrations &raquo; Irreversible migrations](/docs/ORM-ActiveRecord/migrations#irreversible-migrations).
...
See [Validator Options &raquo; strict](/docs/ORM-ActiveRecord/options#strict) for the
declaration syntax.
...
passed to `add`). See [Messages & locales](/docs/ORM-ActiveRecord/i18n).
