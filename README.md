...
These four real sites are the compatibility targets, built on an M-series Mac:

| Site                                           | Full build | Rebuild |
| ---------------------------------------------- | ---------- | ------- |
| behave.dev, 96 pages                           | 58 ms      | 9 ms    |
| blogin.dev, 33 pages                           | 23 ms      | 3 ms    |
| gregdonald.com, 308 pages and 167 MB of images | 705 ms     | 12 ms   |
| keayl.dev, 137 pages                           | 65 ms      | 9 ms    |

A first build of gregdonald.com is mostly copying its images. A rebuild reads
none of them.
...
specs/golden/          expected output, regenerated only on purpose
...
Everything a change needs, in the order you would reach for them:
...
ThreadSanitizer runs the whole suite on both platforms, so a race is a failing
build rather than something noticed later. Where a thread hands work to a
callback the platform runs, teardown has to wait for the callback rather than
assume unregistering stopped it.
...
template. Never inside a parse or render inner loop, where they would become the
contention point the design exists to avoid.
